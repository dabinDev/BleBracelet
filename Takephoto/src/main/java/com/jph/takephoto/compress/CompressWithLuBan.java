...
public class CompressWithLuBan implements CompressImage {
    ...
    private void compressOne() {
        Luban.compress(context, files.get(0)).putGear(Luban.CUSTOM_GEAR).setMaxHeight(options.getMaxHeight()).setMaxWidth(options.getMaxWidth()).setMaxSize(options.getMaxSize() / 1000).launch(new OnCompressListener() {
            @Override
            public void onStart() {
    ...
    private void compressMulti() {
        Luban.compress(context, files).putGear(Luban.CUSTOM_GEAR).setMaxSize(options.getMaxSize() / 1000)                // limit the final image size（unit：Kb）
