...
//        builder.
     ...
     * @param withAnim 是否动画进入
     */
    public void showVoice(String msg, boolean withAnim) {
        inAnim = inDefaultAnim;
        outAnim = outDefaultAnim;
        if (promptView.getCurrentType() != PromptView.PROMPT_LOADING) {
            Builder builder = Builder.getDefaultBuilder();
            builder.icon(R.drawable.ic_laba_voice);
            builder.text(msg);
            promptView.setBuilder(builder);
            closeInput();
            checkLoadView(withAnim);
...
//        window.closeAllPanels();
    ...
    public boolean onBackPressed() {
        ...
        if (isShowing && (promptView.getCurrentType() == PromptView.PROMPT_ALERT_WARN || promptView.getCurrentType() == PromptView.PROMPT_AD)) {
            dismiss();
            return false;
        }
        else {
    ...
    public void showBottomAlert(String title, boolean withAnim, PromptButton... button) {
        inAnim = inSheetAnim;
        outAnim = outSheetAnim;
        Builder builder = Builder.getAlertDefaultBuilder();
        builder.text(title);
        ...
        promptView.setBuilder(builder);
        checkLoadView(withAnim);
        promptView.showBottomSomthingAlert(button);
        dissmissAni(true);
    }
}
