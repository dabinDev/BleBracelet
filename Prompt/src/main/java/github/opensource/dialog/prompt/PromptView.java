 ...
 * Package name:github.opensource.dialog.prompt;
 * Created by :  Benjamin.
...
//画分割线
            ...
            if (button.getRect() == null) {
                button.setTouchRect(new RectF(sheetCellPad * density, canvasHeight - sheetCellPad * density - builder.sheetCellHeight * density, canvasWidth - sheetCellPad * density, canvasHeight - sheetCellPad * density));
            }
            ...
            if (button.isFocus()) {
                ...
                RectF rect = new RectF(sheetCellPad * density, sheetHeight - sheetCellPad * density - builder.sheetCellHeight * density, canvasWidth - sheetCellPad * density, sheetHeight - sheetCellPad * density);
            ...
            bottom = sheetHeight - 1.5f * sheetCellPad * density - builder.sheetCellHeight * density * 1.5f + textRect.height() / 2;
            left = canvasWidth / 2 - textRect.width() / 2;
            if (button.getRect() == null) {
                button.setTouchRect(new RectF(sheetCellPad * density, canvasHeight - 1.5f * sheetCellPad * density - 2f * builder.sheetCellHeight * density, canvasWidth - sheetCellPad * density, canvasHeight - 1.5f * sheetCellPad * density - builder.sheetCellHeight * density));
            }
            canvas.drawText(buttonText, left, bottom, paint);
            if (button.isFocus()) {
                ...
                Rect rectPre = new Rect((int) rect.left, (int) (rect.top - canvasHeight + sheetHeight), (int) rect.right, (int) (rect.bottom - canvasHeight + sheetHeight));
            ...
            for (int i = 2; i < buttons.length; i++) {
                ...
                left = canvasWidth / 2 - textRect.width() / 2;
                if (button.getRect() == null) {
                    button.setTouchRect(new RectF(sheetCellPad * density, canvasHeight - 1.5f * sheetCellPad * density - (i + 1f) * builder.sheetCellHeight * density, canvasWidth - sheetCellPad * density, canvasHeight - 1.5f * sheetCellPad * density - i * builder.sheetCellHeight * density));
                }
                canvas.drawText(buttonText, left, bottom, paint);
...
//        canvas.restore();//别忘了restore
        ...
        if (currentType == PROMPT_ALERT_WARN) {
            ...
            if (buttons.length == 1) {
                ...
                button.setTouchRect(new RectF(transLeft, transTop + top, transLeft + popWidth, transTop + top + buttonH));
                canvas.drawText(buttonText, popWidth / 2 - textRect.width() / 2, top + textRect.height() / 2 + buttonH / 2, paint);
            }
            if (buttons.length > 1) {
...
//                        paint.setAlpha(120);
                    ...
                    button.setTouchRect(new RectF(transLeft + i * buttonW, transTop + top, transLeft + i * buttonW + buttonW, transTop + top + buttonH));
                    canvas.drawText(buttonText, buttonW / 2 - textRect.width() / 2 + i * buttonW, top + textRect.height() / 2 + buttonH / 2, paint);
...
//        promptDialog = null;
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        if (currentType == PROMPT_ALERT_WARN) {
            ...
            if (event.getAction() == MotionEvent.ACTION_UP) {
                if ((drawableClose != null && drawableClose.getBounds().contains((int) event.getX() - transX, (int) event.getY() - transY)) || builder.cancleAble) {
                    promptDialog.dismiss();
                }
                else if (getDrawable() != null && getDrawable().getBounds().contains((int) event.getX() - transX, (int) event.getY() - transY)) {
...
//        currentType = PROMPT_NONE;
    ...
    void showBottomSomthingAlert(PromptButton... button) {
        this.buttons = button;
        showBottomSomthing(PROMPT_ALERT_WARN);
    }

    public void showBottomSomthing(int currentType) {
        this.currentType = currentType;
        isSheet = currentType == PROMPT_ALERT_WARN;
        endAnimator();
        setImageDrawable(getResources().getDrawable(builder.icon));
        width = getDrawable().getMinimumWidth() / 2;
        height = getDrawable().getMinimumHeight() / 2;
