...
class LubanCompresser {
    ...
    private File thirdCompress(@NonNull File file) throws IOException {
        ...
        return compress(filePath, thumb, flip ? thumbH : thumbW, flip ? thumbW : thumbH, angle, (long) size);
    }
    ...
    private File customCompress(@NonNull File file) throws IOException {
        ...
        int angle = getImageSpinAngle(filePath);
        long fileSize = mLuban.maxSize > 0 && mLuban.maxSize < file.length() / 1024 ? mLuban.maxSize : file.length() / 1024;
    ...
    private String getCacheFilePath() {
        StringBuilder name = new StringBuilder(String.valueOf(System.currentTimeMillis()));
    ...
    private File saveImage(String filePath, Bitmap bitmap, long size) throws IOException {
        ...
        if (mByteArrayOutputStream == null) {
            mByteArrayOutputStream = new ByteArrayOutputStream(bitmap.getWidth() * bitmap.getHeight());
        }
        else {
