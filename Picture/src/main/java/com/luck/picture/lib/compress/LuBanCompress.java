...
public class LuBanCompress implements CompressInterface {
    ...
    public LuBanCompress(Context context, CompressConfig config, List<LocalMedia> images, CompressListener listener) {
    ...
    private void compressOne() {
        ...
        Luban.compress(context, files.get(0)).putGear(options.getGrade()).setMaxHeight(options.getMaxHeight()).setMaxWidth(options.getMaxWidth()).setMaxSize(options.getMaxSize() / 1000).launch(new OnCompressListener() {
            @Override
            public void onStart() {
            }
    ...
    private void compressMulti() {
        Log.i("压缩档次::", options.getGrade() + "");
        Luban.compress(context, files).putGear(options.getGrade()).setMaxSize(options.getMaxSize() / 1000)                // limit the final image size（unit：Kb）
                .setMaxHeight(options.getMaxHeight())             // limit image height
                .setMaxWidth(options.getMaxWidth()).launch(new OnMultiCompressListener() {
            @Override
            public void onStart() {
