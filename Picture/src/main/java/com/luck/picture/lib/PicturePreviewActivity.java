        ...
        public Object instantiateItem(ViewGroup container, int position) {
            ...
            if (media != null) {
                ...
                if (isGif && !media.isCompressed()) {
                    ...
                    Glide.with(PicturePreviewActivity.this).asGif().load(path).apply(gifOptions).into(imageView);
                }
                else {
                    RequestOptions options = new RequestOptions().diskCacheStrategy(DiskCacheStrategy.ALL).override(480, 800);
                    Glide.with(PicturePreviewActivity.this).asBitmap().load(path).apply(options).into(imageView);
                }
